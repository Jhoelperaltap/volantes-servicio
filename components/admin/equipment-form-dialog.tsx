...
export function EquipmentFormDialog({ open, onOpenChange, equipment, onSuccess }: EquipmentFormDialogProps) {
  ...
  const fetchEquipmentTypes = async () => {
    try {
      console.log("[v0] Fetching equipment types from /api/admin/equipment-types")
      const response = await fetch("/api/admin/equipment-types")
      console.log("[v0] Equipment types response status:", response.status)

      if (response.ok) {
        const data = await response.json()
        console.log("[v0] Equipment types data received:", data)
        setEquipmentTypes(data)
      } else {
        console.error("[v0] Equipment types fetch failed:", response.status, response.statusText)
      }
    } catch (error) {
